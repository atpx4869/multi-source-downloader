...
class StandardSearchMerger:
    ...
    def merge_results(zby_results: List[Dict], gbw_results: List[Dict]) -> List[Dict]:
        ...
        for zby_result in zby_results:
            zby_key = StandardSearchMerger._normalize_std_no(zby_result.get("std_no", ""))
            ...
            if gbw_result:
                ...
                # 优化：使用浅拷贝而非深拷贝
                merged_result = zby_result.copy()
                
                # 使用 GBW 的数据补充 ZBY（仅在 ZBY 缺失时补充）
                # 优先级顺序：替代标准 > 状态 > 实施日期 > 发布日期
                if not merged_result.get("replace_std") and gbw_result.get("replace_std"):
                    merged_result["replace_std"] = gbw_result["replace_std"]
                
                if not merged_result.get("status") and gbw_result.get("status"):
                    merged_result["status"] = gbw_result["status"]
                
                if not merged_result.get("implement") and gbw_result.get("implement"):
                    merged_result["implement"] = gbw_result["implement"]
                
                if not merged_result.get("publish") and gbw_result.get("publish"):
                    merged_result["publish"] = gbw_result["publish"]
...
if __name__ == "__main__":
    ...
    print("\n\n数据合并测试:")
    zby_data = [
        {"std_no": "GB/T 3100-2015", "name": "社会治安综合治理基础数据规范", "status": "", "implement": "2016-03-01"},
        {"std_no": "GB/T 3101-2015", "name": "标准代号", "status": "", "implement": ""},
    ]
    
    gbw_data = [
        {"std_no": "GB/T 3100-2015", "name": "社会治安综合治理基础数据规范", "status": "现行", "implement": "2016-03-01"},
        {"std_no": "GB/T 3200-2020", "name": "新标准", "status": "即将实施", "implement": "2026-01-01"},
    ]
    ...
    print("\n合并结果:")
    for item in merged:
        print(f"  {item['std_no']}: 状态={item['status']}, 来源={item.get('sources', [])}")
